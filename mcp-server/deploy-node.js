...
if (target !== 'mcp') {
  console.error(`❌ deploy-node.js only supports the 'mcp' target. For staging/prod, use deploy-docker.js.`);
  process.exit(1);
}
...
try {
  ...
  // Tar essential files
  execSync(`tar -czf ${tarFile} dist package.json package-lock.json .env.prod`, { cwd: __dirname, stdio: 'inherit' });
  ...
  if (fs.existsSync(path.join(__dirname, '.env.prod'))) fs.unlinkSync(path.join(__dirname, '.env.prod'));
  process.exit(1);
}
...
conn.on('ready', async () => {
  ...
  try {
    ...
    console.log("2. Checking/Installing dependencies on remote host (NodeJS, Nginx, Certbot)...");
    
    // Check NodeJS 20
    const checkNode = await executeRemoteCommand(conn, "which node || echo 'missing'");
    if (checkNode.stdout.includes("missing")) {
      console.log("Installing Node.js 20 on remote server...");
      await executeRemoteCommand(conn, "curl -fsSL https://deb.nodesource.com/setup_20.x | bash - && apt-get install -y nodejs");
    } else {
      console.log("Node.js is already installed.");
    }

    // Check Nginx
    const checkNginx = await executeRemoteCommand(conn, "which nginx || echo 'missing'");
    if (checkNginx.stdout.includes("missing")) {
      console.log("Installing Nginx on remote server...");
      await executeRemoteCommand(conn, "apt-get update && apt-get install -y nginx");
    }

    // Check Certbot
    const checkCertbot = await executeRemoteCommand(conn, "which certbot || echo 'missing'");
    if (checkCertbot.stdout.includes("missing")) {
      console.log("Installing Certbot Nginx plugin on remote server...");
      await executeRemoteCommand(conn, "apt-get update && apt-get install -y certbot python3-certbot-nginx");
    }
    ...
    await executeRemoteCommand(conn, "mv -f /var/www/vulmini-mcp/.env.prod /var/www/vulmini-mcp/.env");
    ...
    await executeRemoteCommand(conn, "if [ ! -f /swapfile ]; then fallocate -l 1G /swapfile && chmod 600 /swapfile && mkswap /swapfile && swapon /swapfile && echo '/swapfile none swap sw 0 0' >> /etc/fstab; fi");
    await executeRemoteCommand(conn, "cd /var/www/vulmini-mcp && npm ci --omit=dev");
...
`;
    ...
    await executeRemoteCommand(conn, `cat << 'EOF' > /etc/systemd/system/vulmini-mcp.service\n${systemdService}EOF`);
    await executeRemoteCommand(conn, "systemctl daemon-reload && systemctl enable vulmini-mcp && systemctl restart vulmini-mcp");
...
`;
    await executeRemoteCommand(conn, `cat << 'EOF' > /etc/nginx/sites-available/vulmini-mcp\n${nginxConfig}EOF`);
    await executeRemoteCommand(conn, "ln -sf /etc/nginx/sites-available/vulmini-mcp /etc/nginx/sites-enabled/vulmini-mcp");
    await executeRemoteCommand(conn, "rm -f /etc/nginx/sites-enabled/default");
    await executeRemoteCommand(conn, "which ufw && ufw allow 'Nginx Full' || echo 'UFW not installed'");
    ...
    const certbotRes = await executeRemoteCommand(conn, `certbot --nginx -d ${domainName} --non-interactive --agree-tos --email ${sslEmail} --redirect`);
    ...
    conn.end();
    if (fs.existsSync(path.join(__dirname, tarFile))) fs.unlinkSync(path.join(__dirname, tarFile));
    console.log("\nDone!");
