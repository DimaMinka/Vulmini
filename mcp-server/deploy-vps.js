...
async function main() {
  try {
    ...
    console.log(`2. Creating Vultr VPS instance (Ubuntu 24.04, ${plan} in ${region})...`);
    ...
    // Update .env with SSH_HOST and VULTR_PROD_INSTANCE_ID
    console.log("Updating .env file...");
    envContent = envContent.replace(/SSH_HOST=[^\r\n]*/, `SSH_HOST=${ip}`);
    envContent = envContent.replace(/VULTR_PROD_INSTANCE_ID=[^\r\n]*/, `VULTR_PROD_INSTANCE_ID=${instanceId}`);
    fs.writeFileSync(envPath, envContent, 'utf-8');

    console.log("\nDeployment configuration updated successfully in .env:");
    console.log(`- SSH_HOST: ${ip}`);
    console.log(`- VULTR_PROD_INSTANCE_ID: ${instanceId}`);
  } catch (error) {
