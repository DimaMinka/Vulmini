...
const region =
  (regionVar ? getEnvVal(regionVar) : null) ||
  getEnvVal("VULTR_REGION") ||
  "tlv";
const plan =
  getEnvVal(planVar) ||
  (target === "mcp"
...
async function main() {
  try {
    ...
    console.log(
      `2. Creating Vultr VPS instance for target '${target}' (${osName}, ${plan} in ${region})...`,
    );
    ...
    console.log(
      `VPS Instance created: ${instanceId}. Status: ${instance.status}. Waiting to become active...`,
    );
