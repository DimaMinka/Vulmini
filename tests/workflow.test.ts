...
describe("Vulmini Workflow Scenarios", () => {
  ...
  async function pollSnapshot(snapshotId: string) {
    for (let i = 0; i < 20; i++) {
      const snap = await callTool("get_snapshot_status", {
  ...
  async function pollInstance(instanceId: string) {
    for (let i = 0; i < 20; i++) {
      const inst = await callTool("get_instance_status", {
  ...
  it("should complete the full Twin-Instance update loop successfully", async () => {
    ...
    const hostResult = await fetch("http://localhost:3000/mcp", {
      ...
      body: JSON.stringify({
        ...
        method: "tools/call",
        params: {
          name: "set_staging_host",
          arguments: { host: stagingInstance.main_ip },
        },
        id: 99,
      }),
    });
    ...
    const stagingPlugins = await callTool("wp_plugin_status", {
      target: "staging",
    });
    ...
    const prodUpdate = await callTool("wp_run_update", {
      target: "production",
      plugin_slug: "sfwd-lms",
    });
    ...
    const prodMigration = await callTool("wp_db_migrate", {
      target: "production",
    });
    assert.match(prodMigration.output, /Database migrated/);

    // Step 13: Run health check on production
    const prodHealth = await callTool("wp_health_check", {
      target: "production",
    });
    ...
    const stagingExists = listInstances.some(
      (i: any) => i.id === stagingInstance.id,
    );
  ...
  it("should handle multiple concurrent JSON-RPC requests correctly", async () => {
    ...
    for (const res of responses) {
      ...
      assert.strictEqual(data.result.tools.length, 23);
    }
  });
